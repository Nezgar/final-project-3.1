...
void SBomber::CloneGroundObject()
{
    vector<DestroyableGroundObject*> vecDestoyableObjects = FindDestoyableGroundObjects();
    int vecPos = rand() % vecDestoyableObjects.size();
    DestroyableGroundObject* newObj = vecDestoyableObjects[vecPos]->clone();
    double newPos = rand() % 100;
    bool isInside = false;
    do {
        newObj->SetPos(newPos, newObj->GetY());
        for (size_t i = 0; i < vecDestoyableObjects.size(); i++)
        {
            if (vecPos == i)
                continue;
            isInside = vecDestoyableObjects[i]->isInside(newObj->GetX(), newObj->GetY());
        }
        newPos = rand() % 100;
